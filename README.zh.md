...
| [`weekly-investment`](./skills/weekly-investment/SKILL.md) | 生成本地投资组合周报。先调 `portfolio-check` 拿真实持仓快照摘要（Markdown），再按流程输出结构化周报（组合概览 / 收益点评 / 风险警示 / 调仓建议） | ✅ `portfolio-check.mjs`, `pse-review.mjs`, `lib-mcp.mjs` |
...
| `AUTOGEN_PSE_DIR` | 可选：覆盖 autogen-pse 路径（`weekly-investment` 技能用） |
...
│   │   └── SKILL.md
│   └── weekly-investment/  # 投资周报技能
│       ├── SKILL.md
│       └── scripts/
│           ├── lib-mcp.mjs         # 零依赖 MCP stdio 服务器骨架
│           ├── portfolio-check.mjs # MCP 桥接：持仓快照 + 异常扫描
